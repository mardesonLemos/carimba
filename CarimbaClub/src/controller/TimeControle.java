...
public class TimeControle {
	...
	public Time selecionarUm(Time time) {
		...
		try {
			...
			//pse.setString(1, time.getNomeTime());
			if(pse.execute()) {
				ResultSet rse=pse.executeQuery();
				if(rse.next()) {
					retorno=new Time();
					retorno.setIdt(rse.getInt("id"));
					//retorno.setIdTime(rse.getInt("id"));
					retorno.setNome(rse.getString("nome"));
					//retorno.setNomeTime(rse.getString("nome"));
				}
			}
			con.fecharConexao();
			return retorno;
	...
	public boolean inserir(Time time) {
		try {
			...
			pse.setString(1, time.getNome());
			//pse.setString(1, time.getNomeTime44());
			if(!pse.execute()) {
				con.fecharConexao();
				return true;
			}else {
