...
public class UsuarioControle {
  ...
  try { 
	...
	//ps.setString(5, user.getNumeroCamisa())
	ps.setInt(6, user.time.getIdt());
	//ps.setInt(6, user.time.getIdTime());
	if(!ps.execute()) {
	  retorno = true;
	}
	con.fecharConexao();
	...
	if(ps.execute()) {
	  ...
	  if(rs != null) {
		if(rs.next()) {
			...
			//user.setNumeroCamisa(rs.getString("ncamisa"));
			user.time.setIdt(rs.getInt("timeId"));
			//user.time.setIdTime(rs.getInt("timeId"));
					}
				}
			}
			con.fecharConexao();		
	...
	public boolean atualizarTodosTime(Usuario user) {
		try{
            ...
            //ps.setInt(2,user.time.getIdTime());
            if(!ps.execute()) {
	            con.fecharConexao();
	            return true;
            }else {
			...
			if(ps.execute()) {
				...
				if(rs != null) {
					if(rs.next()) {
						...
						//usuario.setNumeroCamisa(rs.getString("ncamisa"));	
						usuario.time.setIdt(rs.getInt("timeId"));
						//usuario.time.setIdTime(rs.getInt("timeId"));
					}
				}
			}else {
				usuario=null;
	...
	public boolean timear(Usuario user){
        try{
            ...
            //usuario.time.setIdTimeq	(rs.getInt("timeId"));
            if(!ps.execute()) {
	            con.fecharConexao();
	            return true;
            }else {
			...
			if(ps.execute()) {
				...
				if(rs != null) {
					...
					while(rs.next()) {						
						...
						//usera.setNumeroCamisa(rs.getString("ncamisa"));
